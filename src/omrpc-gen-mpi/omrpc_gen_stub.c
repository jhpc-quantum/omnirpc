...
generate_rpc_executable(FILE *fp)
{
    ...
    fprintf(fp,"\n");
    fprintf(fp,"#ifndef _MPI_HEADER_H_\n");
    fprintf(fp,"extern void mpi_start_rex(int argc, char **argv);\n");
    fprintf(fp,"extern void mpi_finish_rex();\n");
    fprintf(fp,"#endif\n");
    fprintf(fp,"\n");
    ...
    fprintf(fp, "\tint __tmp,__myrank,__prvd;\n");

    //fprintf(fp,"\tchar nodename[256];\n");
    //fprintf(fp,"\tgethostname(nodename,256);\n");
    //fprintf(fp,"\tprintf(\"nodename = %c%c\\n\",nodename);\n",37,115);
    fprintf(fp,"\tMPI_Init_thread(&argc, &argv, MPI_THREAD_SERIALIZED, &__prvd);\n");
    //fprintf(fp,"\tMPI_Init(&argc, &argv);\n");
    fprintf(fp,"\tmpi_start_rex(argc,argv);\n");
    fprintf(fp,"\tMPI_Comm_rank(MPI_COMM_WORLD,&__myrank);\n");
    fprintf(fp,"\n\tif(__myrank==0) omrpc_stub_INIT(argc,argv);\n");
    fprintf(fp,"\twhile(1){\n");
    fprintf(fp,"\t  if(__myrank==0) __tmp = omrpc_stub_REQ();\n");    
    fprintf(fp,"\t  MPI_Bcast(&__tmp,1,MPI_INT,0,MPI_COMM_WORLD);\n");
    ...
    fprintf(fp,"  MPI_Barrier(MPI_COMM_WORLD);");
    fprintf(fp,"  mpi_finish_rex();\n");
