...
int main(int argc, char **argv) {
  ...
  */
#ifdef USE_OMRPC
  printf("NAS Parallel Benchmarks 2.3-OmniRpc version - EP Benchmark\n");
#else
  printf("NAS Parallel Benchmarks 2.3-serial version - EP Benchmark\n");
#endif
...
#ifdef USE_OMRPC
  printf("set initialize module...\n");
  OmniRpcModuleInit("ep", k_offset, an);
#endif

  for (k = 1; k <= np; k++) {
#ifdef USE_OMRPC
    reqs[k-1] = OmniRpcCallAsync("epOmrpc", k, NQ, 
				 &(r_sx[k-1]), &(r_sy[k-1]), r_q[k-1]);
#else
    kk = k_offset + k;
    t1 = S;
    t2 = an;
